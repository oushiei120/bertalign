...
def compute_similarity_matrix(en_embeddings, ja_embeddings):
    """计算英文和日文句子之间的相似度矩阵"""
    # 归一化嵌入
    ...
    # 计算余弦相似度
    similarity_matrix = np.dot(en_embeddings, ja_embeddings.T)
...
def align_texts(en_file, ja_file, output_file, model_name='LaBSE', batch_size=64, gap_penalty=-0.3, max_chars=None):
    """对齐英文和日文文本文件"""
    ...
    print("计算相似度矩阵...")
    similarity_matrix = compute_similarity_matrix(en_embeddings, ja_embeddings)
    ...
    # 提取对齐结果
    aligned_pairs = []
    for en_idx, ja_idx, similarity in alignments:
        aligned_pairs.append((en_sentences[en_idx], ja_sentences[ja_idx], similarity))
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        for en_sent, ja_sent, similarity in aligned_pairs:
    ...
    # 打印前5个对齐结果
    print("\n前5个对齐结果:")
    for i, (en_sent, ja_sent, similarity) in enumerate(aligned_pairs[:5]):
        print(f"对齐 {i+1}:")
...
def analyze_and_print_statistics(aligned_pairs):
    ...
    # 计算长度比例
    length_ratios = [len(en) / len(ja) if len(ja) > 0 else 0 for en, ja, _ in aligned_pairs]
    ...
    stats = {
        ...
        "length": {
            ...
            "average_ja": avg_ja_length,
            "ratio_en_ja": avg_length_ratio
        }
    }
    ...
    print(f"日文句子平均长度: {stats['length']['average_ja']:.2f} 字符")
    print(f"英日句子长度比例: {stats['length']['ratio_en_ja']:.2f}")
    
    return stats

def main():
    parser = argparse.ArgumentParser(description='英日文本对齐工具')
